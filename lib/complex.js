...
complex.prototype = {
	...
	sub(numToSub){ //subtract number or another complex equation from a complex equation
		var numToSub = this.newDeclare(numToSub);
	...
	conjugate(numToConj){// checks for the conjugate of a complex equation
	...
	equals(eqNum){// check if two or more complex equations are equal
		var eqNum = this.newDeclare(eqNum);
	...
	log(principalValue){ //calculate the logarithm of a complex equation, Log[w] = Log[|w|]+I*(Arg[w]+2*Pi*k)
		if (!principalValue){ // if k is not given find the principal value
			principalValue = 0;
		} 
		var r = Math.log(this.magnitude());
		var i = this.angle() + principalValue * 2 * Math.PI;
		return this.returnResult( 
			Math.round(r * precision)/precision,
			Math.round(i * precision)/precision
			);
...
complex.prototype.newDeclare = function(nreal, nimg){
	if (nreal instanceof complex){ 
	 	return new complex(nreal.real, nreal.img)
	 };
		nreal = +nreal;
		nimg = +nimg;
		return new complex(isNaN(nreal) ? 0 : nreal, isNaN(nimg) ? 0 : nimg);
	};
