...
complex.prototype = {
	...
	multiply: function(numToMultiply){
		...
		numToMultiply = this.newDeclare(numToMultiply);
		var a = this.real;
		var b = this.img;
	...
	divide:function(numToDivide){
		...
		numToDivide = this.newDeclare(numToDivide);
	...
	exp: function(){ //e^(x+i*y) = e^x (Cos[y]+i*Sin[y])= e^x*Cos[y] + i*e^x*Sin[y] 
		real = Math.pow(Math.E,this.real) * Math.cos(this.img);
		img = Math.pow(Math.E,this.real) * Math.sin(this.img);
		return this.returnResult( 
			Math.round(real * 1000000000)/1000000000,
			Math.round(img * 1000000000)/1000000000
			);
	},
	sqrt: function(){ // sqrt(x + iy) = sqrt((xy) + x)/2) + sqrt((xy) - x)/2)
		var abs = this.magnitude(),
			sgn = this.img < 0 ? -1 : 1;
		var real = Math.sqrt((abs + this.real) / 2);
		var img = sgn * Math.sqrt((abs - this.real) / 2);
		return this.returnResult( 
			Math.round(real * 1000000000)/1000000000,
			Math.round(img * 1000000000)/1000000000
			);
	...
	log: function(k){ // Log[w] = Log[|w|]+I*(Arg[w]+2*Pi*k)
		if (!k) k = 0;
		var real = Math.log(this.magnitude());
		var img = this.angle() + k * 2 * Math.PI;
...
complex.prototype.newDeclare = function(real, img){
