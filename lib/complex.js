...
complex.prototype = {
	...
	log(k){ //calculate the logarithm of a complex equation, Log[w] = Log[|w|]+I*(Arg[w]+2*Pi*k)
		if (!k){ // if k is not given find the principal value
			k = 0;
		} 
		var r = Math.log(this.magnitude());
		var i = this.angle() + k * 2 * Math.PI;
		return this.returnResult( 
			Math.round(r * precision)/precision,
			Math.round(i * precision)/precision
			);
